...
def _to_file_uri(path: str) -> str:
    ...
    Ray RLlib (Ray>=2.5x) uses pyarrow fs APIs for checkpoint IO and requires a
    URI scheme. Local filesystem paths must be passed as file:// URIs.
    """
    if not path:
        return path
...
def eval_model(env_name, scenario_name, path2tar, rwf):
    if env_name == "MountainCar":
        ...
        # RLlib checkpoints may require file:// URIs (pyarrow).
...
def train_model(iterations, stop_rw, env_name, scenario_name, path2tar, rwf):
    if env_name == "MountainCar":
        ...
        # Ray>=2.5x expects file:// URIs for checkpoint directories.
