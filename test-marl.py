...
def eval_model2_forced_configs(
    ...
    """Baseline PPO eval, but force reset into each enterprise config (C1..C3).

    This produces a report-ready per-config mean return + success rate.
    """
    env_kwargs = dict(render_mode=None, rw_func=in_rwf, scenario=in_scenario)
    env = cerere_net_v2.env(**env_kwargs)
...
def eval_hmarl_forced_configs(
    ...
    """HMARL PPO eval (RLlib Algorithm.evaluate), forced per enterprise config."""
    ...
    out = {}
    for cfg in configs:
        # Force config via env_options passed to reset().
...
def eval_hmarl_manual_forced_configs(
    ...
    """Manual HMARL eval that forces the enterprise config per episode.

    We evaluate by directly loading RLModules for each HMARL policy from the
    checkpoint and stepping the PettingZoo env (no RLlib Algorithm.evaluate()).

    This gives full control over:
    - forcing reset into C1/C2/C3 via `reset(options={"config_key": ...})`
    - using identical seeds
    - computing a success rate
    """
    ...
    def pick_action(mod: RLModule, obs, action_space):
        ...
        if isinstance(obs, dict):
            # Convert to tensors for RLModule.
            obs_t = {
        ...
        # MultiDiscrete actions (manager): RLlib emits concatenated logits per dimension.
    ...
    for cfg in configs:
        ...
        for ep in range(n_episodes_per_cfg):
            ...
            # Success: critical server not infected.
            crit = getattr(env.unwrapped.base_env, "critserver", None)
            nwstate = getattr(env.unwrapped.base_env, "nwstate", None)
            success = 0 if (crit is not None and nwstate is not None and [1, crit] in nwstate) else 1
            # HMARL env broadcasts same reward to all agents; use manager return as episode return.
